...
let ResponsesService = class ResponsesService {
    ...
    async sendPoll(message) {
        try {
            const response = await axios_1.default.get(`${process.env.SEND_POLL}chat_id=${message.chat_id}&question=${message.question}&options=${JSON.stringify(message.options)}&correct_option_id=${message.correct_option_id}&disable_web_page_preview=true&parse_mode=HTML`);
            return response.data;
        }
        catch (error) {
            return error;
