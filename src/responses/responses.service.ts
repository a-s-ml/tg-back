...
import {
	EditMessageTextDto,
	SendMessageDto
} from "src/webhook-tg/dto/sendMessage,dto"
import {
	EditMessageCaptionDto,
	SendPollDto
} from "src/webhook-tg/dto/sendPoll.dto"
...
export class ResponsesService {
	...
	async editMessageText(message: EditMessageTextDto) {
		try {
			const response = await axios.get(
				`${process.env.SEND_MESSAGE}message_id=${
					message.message_id
				}&text=${message.text}&reply_markup=${JSON.stringify(
					message.reply_markup
				)}&disable_web_page_preview=true&parse_mode=HTML`
			)
			return response.data
	...
	async editMessageCaption(message: EditMessageCaptionDto) {
		try {
			const response = await axios.get(
				`${process.env.SEND_POLL}message_id=${
					message.message_id
				}&caption=${message.caption}&reply_markup=${JSON.stringify(
					message.reply_markup
				)}&disable_web_page_preview=true&parse_mode=HTML`
			)
			return response.data
